...
public class ElazigSuActivity extends AppCompatActivity {

    private RecyclerView recyclerView;
    private List<ElazigSuFaturaModel> elazigSuFaturaModelList;
    private MyAdapterOfElazigSu myAdapterOfElazigSu;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        elazigSuFaturaModelList = new ArrayList<>();

        for(int i=0 ; i<tarih.length; i++) {
            ElazigSuFaturaModel elazigSuFatura = new ElazigSuFaturaModel(tarih[i] , fiyat[i]);
            elazigSuFaturaModelList.add(elazigSuFatura);
        }

        myAdapterOfElazigSu = new MyAdapterOfElazigSu(this , elazigSuFaturaModelList);
        recyclerView.setAdapter(myAdapterOfElazigSu);
